"""In Version 9 , i added qr code color and background color option so that
the qr look according to the theme"""
...
@app.route('/')
def index():
    return render_template('index_color_9.html')
...
def generate():
    ...
    color = request.form.get('color', 'black')  # Get color option, default to black
    background = request.form.get('background', 'white')  # Get background color option, default to white

    # Error handling for empty data
    if not data:
        return render_template('index_color_9.html', error='Please enter data')

    qr = qrcode.QRCode(version=1, box_size=10, border=5)
    qr.add_data(data)
    qr.make(fit=True)

    img = qr.make_image(fill_color=color, back_color=background)  # Use color and background options
    ...
    if heading:
        ...
        draw.text(text_position, heading, fill='red', font=font)
